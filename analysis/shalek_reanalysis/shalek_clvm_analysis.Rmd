...
```{r}
dec_df <- data_frame(gene = featureNames(sce_hvg),
                     m = pcavi$m_beta[1,],
                     s = pcavi$s_beta[1,])
write_csv(dec_df, "../../data/shalek/dec_df.csv")
```
...
textinfo <- frame_data(
  ~x, ~y, ~label,
  1.1, 0.2, "Gene upregulated along pseudotime\nLPS increases upregulation",
  1.1, -0.2, "Gene upregulated along pseudotime\nPAM increases upregulation",
  -1.2, 0.2, "Gene downregulated along pseudotime\nPAM increases downregulation",
...
ints$mgi_symbol <- fData(sce_hvg)$mgi_symbol
...
mm <- match(genes, fData(sce_hvg)$mgi_symbol)
