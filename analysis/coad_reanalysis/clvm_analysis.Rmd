...
df_immune <- mutate(df_beta, is_immune = gene_symbol %in% immune_genes_all) %>% 
  filter(is_immune)
genes_to_plot <- arrange(df_immune, desc(c)) %>% head(n = 18) %>% .$gene_symbol
...
```{r}
lsize = 12
lower_grid <- plot_grid(goplot, quantiseq_plot, labels = c("B", "C"), label_size = lsize)

plot_grid(heatmap, lower_grid, ncol = 1, labels = c("A", ""), label_size = lsize)
ggsave("../../figs/new/coad_1.png", width = 8, height = 5)
```
...
ggsave("../../figs/new/coad_2.png", width = 9, height = 8.5)
```
